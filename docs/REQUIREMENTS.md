# Sekai - 拟真地图生成器 需求文档

> **项目代号**: sekai (日语: 世界)
> 
> **项目定位**: 一个能够自动生成拟真地形环境和政治板块的交互式地图编辑器

---

## 一、项目概述

### 1.1 项目目标

创建一个功能强大的地图生成与编辑工具，能够：

1. **自动生成拟真地形** - 基于程序化生成算法，创建逼真的地形（山脉、平原、海洋、河流等）
2. **生成政治板块** - 自动划分国家/地区边界，模拟真实的政治地理格局
3. **交互式编辑** - 支持用户对生成的地图进行自定义修改
4. **多平台支持** - 同时支持桌面原生应用和 Web 浏览器运行

### 1.2 技术架构

| 组件 | 技术选型 | 说明 |
|------|---------|------|
| 编程语言 | Rust | 高性能、内存安全 |
| GUI 框架 | egui / eframe | 即时模式 GUI，支持 Web 和原生 |
| GPU 渲染 | wgpu | 跨平台图形 API |
| 三角剖分 | delaunator | Delaunay 三角剖分库 |
| Web 编译 | Trunk | WASM 编译与打包工具 |
| 并行计算 | rayon | 数据并行计算 |

### 1.3 项目灵感来源

本项目参考了 [Fantasy Map Generator](https://github.com/Azgaar/Fantasy-Map-Generator) 的设计理念，但使用 Rust + GPU 渲染重新实现，以获得更好的性能和跨平台能力。

---

## 二、核心概念

### 2.1 几何基础

#### 2.1.1 网格系统 (Grid)

地图的基础是一个**抖动网格** (Jittered Grid)：

- 在规则网格的基础上添加随机偏移，避免人工感
- 每个网格点代表一个潜在的 Voronoi 单元中心
- 参数：
  - `width` / `height`: 地图尺寸（逻辑单位）
  - `spacing`: 网格间距，决定单元格密度
  - `jittering`: 抖动强度（默认为 spacing 的 45%）

#### 2.1.2 Delaunay 三角剖分

将网格点连接成三角形网络：

- 保证任意三角形的外接圆内不包含其他点
- 作为 Voronoi 图的对偶图
- 用于地形高度插值和邻接关系计算

#### 2.1.3 Voronoi 图

基于 Delaunay 三角剖分生成：

- 每个 Voronoi 单元对应一个网格点
- 单元边界是相邻三角形外心的连线
- 形成自然、不规则的多边形区域
- 作为地图的基本地理单元（Cell）

```
┌─────────────────────────────────────────┐
│         Voronoi 单元示意图              │
│                                         │
│    ╱╲      ╱╲      ╱╲                   │
│   ╱  ╲    ╱  ╲    ╱  ╲                  │
│  ╱ ·  ╲  ╱ ·  ╲  ╱ ·  ╲   · = 单元中心  │
│  ╲    ╱  ╲    ╱  ╲    ╱                 │
│   ╲  ╱    ╲  ╱    ╲  ╱                  │
│    ╲╱      ╲╱      ╲╱                   │
└─────────────────────────────────────────┘
```

### 2.2 数据模型

#### 2.2.1 单元格数据 (CellsData)

每个 Voronoi 单元存储的属性：

| 属性 | 类型 | 说明 |
|------|------|------|
| `height` | u8 | 高度值 (0-255)，决定地形类型 |
| `temperature` | i8 | 温度 (待实现) |
| `precipitation` | u8 | 降水量 (待实现) |
| `biome` | u16 | 生物群落 ID (待实现) |
| `state` | u16 | 国家/政治实体 ID (待实现) |
| `culture` | u16 | 文化区域 ID (待实现) |
| `religion` | u16 | 宗教区域 ID (待实现) |
| `province` | u16 | 省份/行政区 ID (待实现) |

#### 2.2.2 特征系统 (Feature System)

##### 单元格特征 (CellFeature)

作用于一组 Voronoi 单元的特征：

| 特征类型 | 说明 | 示例 |
|---------|------|------|
| `State` | 国家/政治实体 | 帝国、王国、共和国 |
| `Culture` | 文化区域 | 语言、习俗相近的区域 |
| `Religion` | 宗教区域 | 信仰分布 |
| `Province` | 省份/行政区 | 国家内部的行政划分 |
| `Biome` | 生物群落 | 森林、草原、沙漠、冻原 |

##### 点特征 (PointFeature)

作用于单个点的特征：

| 特征类型 | 说明 | 人口规模 |
|---------|------|---------|
| `City` | 城市 | > 100,000 |
| `Town` | 城镇 | 10,000 - 100,000 |
| `Village` | 村庄 | 1,000 - 10,000 |
| `Settlement` | 定居点 | < 1,000 |

---

## 三、功能需求

### 3.1 地形生成 (Phase 1) 🚧

#### 3.1.1 海陆分布

- [ ] 基于噪声函数生成基础高度图
- [ ] 定义海平面高度阈值
- [ ] 生成大陆和海洋的基本轮廓
- [ ] 支持岛屿生成

#### 3.1.2 地形起伏

- [ ] 山脉生成算法（板块碰撞模拟或噪声叠加）
- [ ] 平原、丘陵、高原分布
- [ ] 海沟和海底地形
- [ ] 地形平滑处理

#### 3.1.3 水系生成

- [ ] 河流生成（从高处向低处流动）
- [ ] 湖泊生成（凹陷地形积水）
- [ ] 三角洲和河口
- [ ] 瀑布和急流

### 3.2 气候系统 (Phase 2)

#### 3.2.1 温度分布

- [ ] 纬度对温度的影响
- [ ] 海拔对温度的影响
- [ ] 洋流对沿海温度的影响
- [ ] 季节性温度变化

#### 3.2.2 降水分布

- [ ] 信风和季风模式
- [ ] 地形对降水的影响（雨影效应）
- [ ] 沿海与内陆的降水差异

#### 3.2.3 生物群落

基于温度和降水自动确定：

| 生物群落 | 温度 | 降水 |
|---------|------|------|
| 热带雨林 | 高 | 高 |
| 热带草原 | 高 | 中 |
| 沙漠 | 高/低 | 低 |
| 温带森林 | 中 | 中-高 |
| 温带草原 | 中 | 低-中 |
| 针叶林 | 低 | 中 |
| 苔原 | 极低 | 低 |
| 冰原 | 极低 | 任意 |

### 3.3 政治板块生成 (Phase 3)

#### 3.3.1 文化区域

- [ ] 基于地理隔离（山脉、河流、海洋）划分文化区
- [ ] 文化扩散模拟
- [ ] 文化融合与边界过渡

#### 3.3.2 国家生成

- [ ] 初始国家种子点放置
- [ ] 国家扩张模拟（考虑地形、人口、资源）
- [ ] 边界稳定化
- [ ] 飞地和包围地处理

#### 3.3.3 行政区划

- [ ] 省份自动划分
- [ ] 首都和主要城市确定
- [ ] 边境城镇标记

### 3.4 城镇系统 (Phase 4)

#### 3.4.1 定居点生成

- [ ] 基于资源、水源、贸易路线放置城市
- [ ] 人口分布模拟
- [ ] 城市规模分级

#### 3.4.2 道路网络

- [ ] 城市间道路连接
- [ ] 贸易路线
- [ ] 港口和航线

### 3.5 地图渲染 (当前进度 ✅)

#### 3.5.1 基础渲染 ✅

- [x] 网格点渲染
- [x] Delaunay 三角剖分线框渲染
- [x] Voronoi 边界渲染
- [x] GPU 加速渲染 (wgpu)
- [x] 着色器 (WGSL)

#### 3.5.2 图层系统

- [ ] 多图层渲染架构
- [ ] 图层可见性控制
- [ ] 图层透明度
- [ ] 图层渲染顺序

#### 3.5.3 地形可视化

- [ ] 高度图着色
- [ ] 等高线渲染
- [ ] 山体阴影
- [ ] 水域着色

#### 3.5.4 政治地图可视化

- [ ] 国家边界线
- [ ] 国家填充色
- [ ] 省份边界
- [ ] 城市图标和名称

### 3.6 交互功能 (当前进度 ✅)

#### 3.6.1 画布操作 ✅

- [x] 平移 (Pan) - 空格键 + 拖拽 或 滚轮
- [x] 缩放 (Zoom) - 鼠标滚轮 / 触控板捏合
- [x] 缩放限制 (0.1x - 100x)

#### 3.6.2 编辑工具 (待实现)

- [ ] 笔刷工具 - 绘制地形
- [ ] 橡皮擦 - 删除地形
- [ ] 选择工具 - 选中单元格
- [ ] 填充工具 - 区域填充

#### 3.6.3 信息查看

- [ ] 悬停显示单元格信息
- [ ] 点击选中单元格详情
- [ ] 坐标显示

---

## 四、UI 设计需求

### 4.1 主界面布局

```
┌────────────────────────────────────────────────────────┐
│ [File] [Edit] [View] [Generate] [Help]    🌓 主题切换  │
├──────────────┬─────────────────────────────────────────┤
│              │                                         │
│   工具面板   │                                         │
│   --------   │                                         │
│   🖌️ 笔刷    │                                         │
│   🧽 橡皮    │           地 图 画 布                   │
│   🔲 选择    │                                         │
│   🪣 填充    │                                         │
│              │                                         │
│   --------   │                                         │
│   图层列表   │                                         │
│   ☑ 地形    │                                         │
│   ☑ 政治    │                                         │
│   ☑ 城市    │                                         │
│              │                                         │
├──────────────┴─────────────────────────────────────────┤
│ 状态栏: 坐标 (x, y) | 缩放 100% | 选中: Cell #1234    │
└────────────────────────────────────────────────────────┘
```

### 4.2 生成配置面板

```
┌─────────────────────────────────┐
│ 🗺️ 地图生成设置                 │
├─────────────────────────────────┤
│ 地图尺寸                        │
│   宽度: [____2000____] px       │
│   高度: [____1000____] px       │
│                                 │
│ 网格密度                        │
│   间距: [_____10_____] px       │
│   预估单元格: ~20,000           │
│                                 │
│ 地形设置                        │
│   海陆比例: [====●====] 40%     │
│   山地比例: [==●======] 20%     │
│                                 │
│ 种子值                          │
│   [________12345________] 🎲    │
│                                 │
│        [  生成地图  ]           │
└─────────────────────────────────┘
```

---

## 五、性能需求

### 5.1 渲染性能

| 指标 | 目标值 | 说明 |
|------|-------|------|
| 帧率 | ≥ 60 FPS | 正常交互情况下 |
| 单元格数量 | 支持 50,000+ | 不明显卡顿 |
| 初始加载 | < 3 秒 | 包含地图生成 |

### 5.2 内存使用

| 场景 | 预期内存 |
|------|---------|
| 10,000 单元格 | < 50 MB |
| 50,000 单元格 | < 200 MB |
| 100,000 单元格 | < 500 MB |

### 5.3 优化策略

- [x] 视口裁剪 - 只渲染可见区域的几何体
- [x] GPU 加速 - 使用 wgpu 进行渲染
- [x] 并行计算 - 使用 rayon 加速 CPU 计算
- [ ] LOD (Level of Detail) - 缩小时减少细节
- [x] 空间索引 - 加速单元格查询和视口裁剪

---

## 六、项目结构

```
sekai/
├── src/
│   ├── main.rs              # 应用入口
│   ├── lib.rs               # 库入口
│   ├── app.rs               # 主应用结构
│   │
│   ├── delaunay/            # Delaunay 三角剖分
│   │   ├── mod.rs
│   │   ├── delaunay.rs      # 三角剖分算法
│   │   ├── voronoi.rs       # Voronoi 图生成
│   │   ├── triangle.rs      # 三角形数据结构
│   │   └── utils.rs         # 工具函数
│   │
│   ├── gpu/                 # GPU 渲染模块
│   │   ├── mod.rs
│   │   ├── canvas_uniform.rs
│   │   ├── map_renderer.rs
│   │   ├── points_renderer.rs
│   │   ├── delaunay/        # Delaunay 渲染
│   │   └── voronoi/         # Voronoi 渲染
│   │
│   ├── models/              # 数据模型
│   │   ├── mod.rs
│   │   ├── map/
│   │   │   ├── grid.rs      # 网格系统
│   │   │   ├── system.rs    # 地图系统
│   │   │   ├── cells_data.rs # 单元格数据
│   │   │   └── feature.rs   # 特征系统
│   │   └── map_layer.rs     # 图层模型
│   │
│   ├── map_layer/           # 图层定义
│   │   ├── mod.rs
│   │   └── layer_def.rs     # 图层 trait
│   │
│   ├── resource/            # 资源管理
│   │   ├── mod.rs
│   │   └── resource_impl.rs # 线程安全资源
│   │
│   └── ui/                  # UI 组件
│       ├── mod.rs
│       ├── canvas/          # 画布组件
│       │   ├── canvas.rs
│       │   ├── state.rs     # 画布状态
│       │   ├── widget_impl.rs
│       │   └── input/       # 输入处理
│       └── map/             # 地图 UI
│
├── assets/
│   └── shaders/             # GPU 着色器
│       ├── points.wgsl
│       ├── delaunay.wgsl
│       └── voronoi.wgsl
│
├── docs/                    # 文档
│   └── REQUIREMENTS.md      # 本文档
│
├── benches/                 # 性能测试
│   └── delaunay.rs
│
├── Cargo.toml               # 依赖配置
├── Trunk.toml               # Web 构建配置
└── README.md
```

---

## 七、开发路线图

### Phase 1: 基础框架 ✅ (当前)

- [x] 项目初始化
- [x] egui + wgpu 集成
- [x] 网格点生成
- [x] Delaunay 三角剖分
- [x] Voronoi 图生成
- [x] GPU 渲染管线
- [x] 画布平移/缩放

### Phase 2: 地形生成 (进行中)

- [ ] 高度图生成算法
- [ ] 海陆分布
- [ ] 高度着色渲染
- [ ] 水系生成

### Phase 3: 气候与生物群落

- [ ] 温度计算
- [ ] 降水计算
- [ ] 生物群落分配
- [ ] 生物群落可视化

### Phase 4: 政治地理

- [ ] 文化区域生成
- [ ] 国家生成
- [ ] 省份划分
- [ ] 边界渲染

### Phase 5: 城镇与路网

- [ ] 城市放置算法
- [ ] 人口分布
- [ ] 道路网络
- [ ] 标注系统

### Phase 6: 编辑功能

- [ ] 笔刷工具
- [ ] 选择工具
- [ ] 撤销/重做
- [ ] 导出功能

---

## 八、参考资料

### 8.1 算法参考

- [Polygonal Map Generation](http://www-cs-students.stanford.edu/~amitp/game-programming/polygon-map-generation/) - Amit Patel
- [Fantasy Map Generator](https://github.com/Azgaar/Fantasy-Map-Generator) - Azgaar
- [Procedural World Generation](https://www.redblobgames.com/) - Red Blob Games

### 8.2 技术文档

- [egui 文档](https://docs.rs/egui)
- [wgpu 文档](https://docs.rs/wgpu)
- [delaunator-rs](https://docs.rs/delaunator)

---

## 九、术语表

| 术语 | 英文 | 说明 |
|------|------|------|
| 单元格 | Cell | Voronoi 图中的一个多边形区域 |
| 三角剖分 | Triangulation | 将点集划分为不重叠三角形的过程 |
| 外心 | Circumcenter | 三角形外接圆的圆心 |
| 生物群落 | Biome | 具有相似气候和生态特征的区域 |
| 图层 | Layer | 地图的一个可视化层级 |
| 抖动 | Jittering | 向规则网格添加随机偏移 |

---

*文档版本: 1.0*
*最后更新: 2026-01-11*
