...
密度由**屏幕间距**而不是格元份额决定（2026-09-02 修订）：三档密度是目标间距
低 48 / 中 32 / 高 20 px（参考画布宽 1280 px）；缓存键是「档位 + `⌊log2(缩放)⌋`
带」，采样分母在构建时按格元数取满足目标的最小 2 的幂，因此 Draft 与 Standard
在同一档下屏幕上的箭头间距相同，缩放每翻一倍分母减半、只加不减。此前固定的
1/16、1/8、1/4 份额在 Standard 档放出上万个箭头，整屏不可读。
