...
> 已被 `2026-08-10-procedural-spherical-tectonic-heightmap-design.md` 取代：本文件中的 field-weighted 最终板块分区、大陆区域生长与对应高度组合不再作为生产方向。闭合球面、唯一事实源、无投影物理、无高度顶点位移、确定性与模块正交约束继续有效。

日期：2026-08-10
状态：历史实现；生产方向已由上方新设计取代
...
FieldBand 保存角尺度 theta、相对权重和独立 seed；采样坐标为 p / theta_rad 加一个由 seed 决定的三维偏移。第一版固定 recipe 为：

| 字段 | 角尺度与归一化权重 |
|---|---|
| plate resistance | 100°／0.55，42°／0.30，16°／0.15 |
| plate fabric | 75°／0.65，28°／0.35，8°／0.60 |
...
- k 大于等于 8 时先生成一个从 0.55 到 1.90 的稳定秩序轮廓，再叠加不超过正负 10% 的标签随机扰动；若离散样本的目标面积变异系数低于 0.36，则只沿其相对均值的偏差做确定性放大并继续夹在 0.45–2.40 内；归一化后仍保持最大／最小目标比不低于 2.75；
...
种子最终仍落在权威 CellId 上，以保证稳定身份和连通传播；候选布局先在连续单位球上确定，避免网格分辨率改变候选集合。选择过程不再简单地反复取全局最远点：

1. 按目标面积从大到小安置板块；
2. 用目标面积的平方根估计等效角半径；
3. 从 plate-seed-placement-v2 一次性生成 1,024 个均匀连续球面方向；
4. 候选评分使用与已选连续方向的“大圆距离／两者等效半径和”；
5. 从仍满足分离约束的最高 5% 候选中使用标签随机索引选择，而不是永远取唯一最大值；
6. 方向选定后才吸附到最近的尚未使用权威单元；后续传播和面积校准只使用这个 CellId。

该策略保留必要的分离度，但不制造规则、近等距的种子格局。连续方向与随机流在 5k／20k 网格间相同，吸附误差只允许落在粗网格典型单元直径内。
...
为每条权威邻接边 e 构造一个共同的正代价：
    ...
    cost(e) = length(e) × clamp(
        1 + 0.45 × resistance(e) + 1.50 × fabric_crossing(e),
        0.45,
        2.20
    )
...
首版设计冻结的是算法结构、物理尺度和验收区间，而不是允许已知失败穿过验收门槛的常数。复审时用同一套 17-seed 形态 oracle 和 5k／20k 分辨率 oracle 对首版常数做了逐项回放，得到以下有界修订：

- plate target 的标签扰动从正负 20% 收窄到正负 10%，并在离散目标 CV 低于 0.36 时只沿相对均值偏差做最多 16 次确定性放大；浮点夹限和十亿整数权重归一化后都再次验证 CV 后置条件。首版正负 20% 在 seed 4 产生 `max/min=2.4886`、`CV=0.2766`，在 seed 9 只产生 3/12 个主轴长宽比大于 1.25 的板块；修订后全部 17 个默认种子以及 `[8,12,32,64] × 1024 seeds` 的目标生成矩阵满足既有门槛。
- fabric crossing 系数从 1.00 调整为 1.50，但仍受原有 0.45–2.20 正代价夹限。把它恢复到 1.00 时，seed 0、10、15 分别只有 4、5、5 个板块超过 1.25 长宽比，违反“每个种子至少半数板块拉长”的既有门槛。
- soft plate-interior preference 从 0.15 调整为 0.10。恢复到 0.15 时，多种子陆海门槛仍通过，但 5k／20k 总归一化海岸周长差异为 15.04%，超过既有 15% 分辨率门槛；0.10 时为 13.01%。
- 区域前沿增加按物理特征半径缩放的局部凝聚项，系数为 80,000。把该项归零时，5k／20k 总归一化海岸周长差异升到 23.32%；保留该项时不改变目标陆地面积，只把同一物理尺度的局部前沿排序稳定在不同网格分辨率之间。

这些修订不增加新字段、状态或用户参数，也不改变依赖方向。它们只校准现有场、度量和亲和组合；所有硬质量门槛保持原值，未为接受结果而放宽。
...
大陆不是板块的子集，也不等于一个或多个板块。大陆亲和分数由以下正交分量组合：
    ...
    affinity =
      ...
      + 0.10 × soft_plate_interior_preference
...
7. 海岸收缩前为每个已选分量只构建一次由深部锚点指向海岸的生成树；保护所有领域种子和每棵树的深部根，只把当前海岸树叶放入稳定优先队列。每删除一个树叶，仅更新其父节点和图邻居的叶／海岸状态，不做逐 cell 全图搜索；删除任意树叶前缀都保持剩余树连通。

所有阈值以全球面积比例、平方米或球面角尺度表示，不以“几个 cell”表示。最终误差不得超过一个权威单元面积。

区域前沿的局部凝聚项按“特征物理面积／中位单元面积”的平方根缩放，因此 5k 与 20k 网格表达同一个物理特征时会获得相近的边界平滑压力，而不是固定奖励若干个相邻 cell。该项只调整候选前沿的局部排序，不建立第二份地表、历史状态或持久缓存。
...
- 海洋厚度使用独立种子和独立的 14°／1.00 中尺度场，不复用大陆 36°／0.70 + 14°／0.30 场；
- 场振幅被现有最小／最大厚度常量夹紧；
...
并在同一自然生成器层增加两个不含领域策略的共享原语：

    src/generators/natural/fractal.rs
    src/generators/natural/connectivity.rs

- fractal.rs 只定义有界分形频带参数，供球面形态场和既有 relief 噪声共同使用；
- connectivity.rs 只定义稳定并查集和规范化 PlateId 对，供平面与球面边界聚合共同使用。
...
- 板块／海岸重合率：以海岸和板块边界各向外半个中位 cell 角直径形成总宽一个中位 cell 角直径的缓冲带，统计落入该带的海岸弧长比例。
...
- 20k 板块归一化周长相对 5k 增加 4%–15%，证明可解析细节出现但形态没有退化成网格噪声；
- 结构尺度不会随 cell count 增加而缩成单元级噪声；
- 把 20k 单元中心最近邻重采样到 5k 后，经最优 PlateId 匹配的 plate owner 面积加权一致率不低于 90%；
- 同样重采样后的大陆 mask 面积加权 Jaccard 不低于 65%。

不要求 CellId 一一相同，因为两个地表本来具有不同 SurfaceRef。板块归属承担强分辨率稳定性；大陆面积掩膜还要做离散连通区域增长，因此用较低但仍占多数的 Jaccard 下限，同时由主要陆块数、海岸周长、面积与视觉门禁共同约束，不能单独降低某项来接受圆团。
