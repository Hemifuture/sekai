...
结论：**在地球水量下，陆地占比有上限**。Task 1 实测（§2.4）：五个预设
在地球水量下的陆地中位只有 0.16–0.23（单粒最高 0.29），湿面积均深
3.1–3.4 km；陆壳从 0.16 加到 0.42，陆地几乎不动（几何变了，淹没程度
没变）。陆壳比例再高也越不过这条线——多出来的陆壳把水挤高，反过来
...
- 露出率 = 陆地 / 陆壳：地球 0.71，本世界 0.53。差异的已测部分：
  ...
  （≥ 44 km 份额 0.031 对 CRUST1.0 0.115）。归因见 §2.4（A）：−3.3 km
  **全部**是裂谷拉张预算的面积稀释，体积、消耗、重采样三项为零。
...
### 2.4 Task 1 实测（2026-08-22，`probe_t0b_land_fraction_driver`，草稿档 17 粒）

**（A）陆壳清单均值去向**（Continents，`SphericalTectonicMaterialBudget`
账本，17 粒中位；单位 km）：

| 项 | 值 | 说明 |
| --- | ---: | --- |
| 初始均值 V₀/A₀ | 38.66 | = CRUST1.0 台地表均值（秩映射精确再现） |
| 裂谷拉张面积增量 | +0.150 A₀ | `MAXIMUM_RIFT_EXTENSION_AREA_FRACTION`，17 粒**全部用满** |
| 碰撞缩短面积回收 | −0.057 A₀（0.038–0.137） | 纯剪切缩短 |
| 大陆消耗 | 0 | 无大陆物质被消耗 |
| 面积稀释项 V₀/A₁ − V₀/A₀ | **−3.28**（−3.89…−0.51） | 体积守恒下面积 +9% 的必然结果 |
| 体积损失项 (V₁−V₀)/A₁ | 0.00 | 体积严格守恒 |
| 重采样项 V₂/A₂ − V₁/A₁ | 0.00 | 权威面与控制面均值一致 |
| 终态均值 | 35.38（34.78–38.15） | = 38.66 × A₀/A₁ |

结论：清单均值下降 **100% 来自裂谷预算的面积稀释**，不是混合、不是
消耗。对照地球：CRUST1.0 全部陆壳均值 36.1 km 对台地 38.7 km，等价于
面积 +7%（0.933 倍）；v5 的 +15% 约为地球当量的两倍。把该预算改为
地球当量（0.07）可把终态均值抬到 ≈ 36.4 km（+1 km ⇒ 干舷 +150 m），
是一个有出处、可过程实现的修正，但只是次级杠杆——露出率差距的主体仍
是厚度分布的厚尾不足（T0 开放项）。本里程碑不改 v5 预算（§4），
记录为后续候选。

**（B）各预设在地球水量下的陆地与水量响应**（17 粒中位，r = 水量 /
地球缩放水量）：

| 预设 | 陆壳（演化后） | 海面 m | 陆地 L(r=1) | 露出率 | 湿面积均深 km |
| --- | ---: | ---: | ---: | ---: | ---: |
| Continents | 0.38（0.415） | −101 | **0.202**（0.167–0.292） | 0.486 | 3.28 |
| Archipelago | 0.26（0.278） | −699 | **0.223**（0.195–0.243） | 0.785 | 3.37 |
| Supercontinent | 0.42（0.462） | +57 | **0.174**（0.137–0.285） | 0.373 | 3.17 |
| GreatIsland | 0.28（0.304） | −643 | **0.230**（0.191–0.252） | 0.747 | 3.40 |
| VolcanicIslands | 0.16（0.167） | −1271 | **0.158**（0.153–0.166） | 0.926 | 3.11 |

陆地随水量的曲线 L(r)（中位）：

| 预设 | r=0.25 | 0.5 | 0.75 | 1.0 | 1.5 | 2.0 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| Continents | 0.472 | 0.409 | 0.340 | 0.202 | 0.026 | 0.006 |
| Archipelago | 0.382 | 0.295 | 0.265 | 0.223 | 0.065 | 0.010 |
| Supercontinent | 0.510 | 0.447 | 0.342 | 0.174 | 0.018 | 0.004 |
| GreatIsland | 0.391 | 0.314 | 0.284 | 0.230 | 0.059 | 0.008 |
| VolcanicIslands | 0.301 | 0.206 | 0.173 | 0.158 | 0.099 | 0.023 |

达到给定陆地目标所需的隐含水量比 r(T)，括号内为该目标下露出洋底占
陆地的份额（中位）：

| 预设 | T = 标称 | 0.29 | 0.38 | 0.50 | 0.60 |
| --- | ---: | ---: | ---: | ---: | ---: |
| Continents（标称 0.38） | 0.624 (0.023) | 0.848 (0.007) | 0.624 (0.023) | 0.195 (0.178) | 0.096 (0.311) |
| Archipelago（0.26） | 0.795 (0.025) | 0.543 (0.063) | 0.253 (0.271) | 0.122 (0.445) | 0.065 (0.538) |
| Supercontinent（0.42） | 0.585 (0.018) | 0.831 (0.004) | 0.686 (0.009) | 0.282 (0.088) | 0.114 (0.234) |
| GreatIsland（0.28） | 0.770 (0.022) | 0.713 (0.029) | 0.270 (0.206) | 0.130 (0.394) | 0.064 (0.495) |
| VolcanicIslands（0.16） | 0.968 (0.030) | 0.267 (0.425) | 0.167 (0.561) | 0.084 (0.666) | 0.046 (0.721) |

读数：

1. 地球水量下五个预设的陆地都落在 0.16–0.23：陆壳比例决定几何，
   **不**决定陆地占比——§2.3 的分工由数据坐实。
2. 每个预设的标称陆地都可在 r = 0.59–0.97 达到，且露出洋底 ≤ 3%；
   预设意图不需要动陆壳，只需要少一点水。
3. 陆地对水量在 r = 1–1.5 区间极陡（Continents 0.20 → 0.03）：大陆
   测高在海面附近很平。后期的水量旋钮应以对数刻度或直接以陆地目标
   呈现；目标模式天然规避了这种敏感性。
4. 露出洋底的阈值：目标低于 **0.9 × 演化后陆壳面积份额** 时洋底份额
   ≤ 3%，越过后迅速上升（Continents 0.50 → 18%，0.60 → 31%）。UI 提示
   以此阈值触发。
...
| 常量 | 拟值 | 依据 |
| --- | ---: | --- |
| `recommended_land_fraction` Continents | 0.20 | §2.4（B）中位 0.202 |
| … Archipelago | 0.22 | 0.223 |
| … Supercontinent | 0.17 | 0.174 |
| … GreatIsland | 0.23 | 0.230 |
| … VolcanicIslands | 0.16 | 0.158 |
| `MIN_WATER_INVENTORY_RATIO` / `MAX_…` | 0.05 / 5.0 | 0.05 仍保有海洋（L ≈ 0.6–0.65）；5.0 已是水世界（L < 0.01） |
| `WATER_INVENTORY_RATIO_ADVISORY_MIN` / `MAX` | 0.5 / 2.0 | Continents L ≈ 0.41 … 0.006；仅提示 |
| 露出洋底提示阈值 | 目标 > 0.9 × 演化后陆壳面积份额 | §2.4 读数 4 |

目标模式的隐含水量比是输出，不受 `MIN/MAX_WATER_INVENTORY_RATIO`
约束（用户裁定：结果归玩家）；超出建议带时 UI 提示。
