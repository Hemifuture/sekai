...
日期：2026-09-02
状态：**已实施**（用户 2026-09-02 指示修正"风场完全对称"；Task 0–3 完成，实测见 §6.1）
...
| | 基线 | Task 1 地形 | Task 1 + 2 摩擦（ρ = 3） |
| --- | ---: | ---: | ---: |
| 非纬向方差占比（seed 42） | 0.014 | 0.034 | 0.046 |
| 17 seed 均值（范围） | — | — | 0.047（0.041–0.059） |
| 陆 / 海偏离均方根 m/s | 0.93 / 0.92 | 2.10 / 1.10 | 1.52 / 1.16 |
| 17.5° 信风 u（北 / 南） | −12.4 / −12.4 | −12.0 / −12.1 | −8.4 / −9.3 |

机制正确、参数有出处、幅度未校准：占比从 1.4 % 到 4.7 %，仍比地球低一个量级。
剩余对称性来自 §2 的成因 1、4、5（纯纬度平衡温度、轴对称闭合、6–9 日瞬态）。
副作用：更真实的信风让全球降水 2.80 → 2.56 mm/day，暴露出此前与 GPCP 的吻合
依赖偏强的风；绕山气流使地形雨影度量失败增多。两者都是 P4 水热校正的输入，
不在本里程碑做事后修形。
