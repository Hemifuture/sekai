...
- Stage identity RED/GREEN: after the shared-fracture scientific change, the focused contract failed with stage version `3` versus expected `4`. `SphericalTectonicStage` and every production-stream fixture now use version `4`, so StageCache/RNG/reproducibility identities cannot restore version-3 tectonic output under the new algorithm.
- Initial-crust compliance: 17 seeds × `[0.20, 0.38, 0.55]` hit the requested area within one maximum cell and preserve nested masks. Version-4 post-evolution paired means are `[0.1111613102, 0.2216897322, 0.3163841438]`; adjacent paired median deltas are `[0.1161700162, 0.0862142107]`.
...
- Release 20,252-cell land compliance: seed 3 `0.379971/-1555.50 m`; seed 7 `0.380033/-1534.75 m`; seed 11 `0.379986/-1599.00 m`; seed 19 `0.380002/-1599.75 m`; seed 42 `0.380003/-1626.25 m`.
- Release performance against the frozen untouched baseline `1418.187 ms`: planar `1941.047 ms`, spherical `1552.889 ms`, isolated morphology tectonics `237.454 ms`, formal tectonic stage `328.618 ms`, morphology peak delta `11,997,184 B`, persistent spherical artifacts `23,098,336 B`, sphere/planar ratio `0.800026`. All frozen budgets pass. The independent 4,842/20,252-cell morphology-resolution gate also passes.
- Native v4 smoke at logical `1280×720`: 20,252 cells; map/globe elevation-only, wind-only, and combined views all rendered; target/actual `38.0/38.0` and `55.0/55.0` rebuilt correctly while the same world's requested/evolved continental crust remained independently `38.0/18.5`. Sea level moved from `-2378.8 m` to `-3005.2 m`. The terrain has irregular coherent continents and ridged relief rather than Voronoi/hexagonal outlines; the globe remains a round undeformed unit sphere.
- Chrome 150 v4 fresh-release smoke at logical `1280×720`: seed 42 produced 20,252 cells, requested/evolved crust `38.0/19.9`, target/actual land `38.0/38.0`, sea level `-1673.5 m`, all three layer controls, map/globe, and independent elevation/wind visibility. Web distribution contract is `4/4`: activation preserves unrelated origin caches and offline fallback reads only Sekai's named cache. `trunk build --release --no-sri true` succeeds from current source.
- Frozen field hash: `0d80ce4151f981cba52ca7fb7dfb38515959e2f7394509a9647830499518f0af`. Frozen full-graph result hash: `f2fb479d93d5c63a2180bed8bec6babe597658b47a7fa902584bb513781b7d83`; exact repeated artifact hashes are stored in `spherical_natural_stage_graph.rs` and pass twice consecutively.
- Audited RTX 4080 SUPER Vulkan and GL RGBA8 hashes are byte-identical: scalar map/globe `ba9a9ffa…b8c9`/`11db743d…7559`; category `be02bbbe…2288`/`83f1ed37…d060`; edge scalar `c675a690…8ad2`/`c36ab587…664b`; edge category `7a73e831…954`/`2f75ab87…9fe9`; vector paused map/globe `e57496e9…f668`/`c29e422f…0a82`; animated `2f860794…a69`/`baa1a0b6…caa1`; seam `0296847b…d9e7`; poles `45954cc3…5dab`; front/back `83f1ed37…d060`/`3cd1ed9e…8e91`. CPU semantic oracles run before every hash assertion.
- Fresh gates after the final code delta: fmt, workspace check, strict all-target/all-feature Clippy, wasm32 all-features, Web Release build, and diff check exit 0; workspace all-target/all-feature tests exit 0 in `385.1 s` with 416 library tests plus all integration/bench targets; doctests `5 passed/8 ignored`; required-GPU full suite exits 0 in `309.4 s` with no GPU skip; Vulkan and GL public golden suites and renderer suites are all green.
