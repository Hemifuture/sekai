...
- [x] Task 8 —— 压缩比上限重测（设计 §6.6）
      Task 6 修掉配对门的病态归一后，卡住压缩比的两个障碍去掉了一个，重扫工作点。
      结论：**上限没有移动，仍是 30。** 60 在 seed 42 的 Draft 与 Standard 上都跑得
      通、时延还更好（16.1 / 52.8 s），但 Draft 的 32 seed 扫描失败（Standard 的
      通过）；100 过 Draft、Standard 局部径流 4050 破 4000 上限；200 Draft 即失控。
      两条经验：约束来自扫描而非单个种子；破限与分辨率不单调，由种子与地形决定，
      两档都要各自跑扫描。顺带查明：降水从 2.85 抬到 3.13 不是压缩造成的（60 与 100 同值），是
      海洋真正达到平衡后的平衡性质，压缩 30 的降水含海温欠收敛的补偿误差，
      嫌疑在蒸发用的是 6 km 板平均风而非近地面风（设计 §6.6.1，留给下一里程碑）。

## 用户验证步骤

字段目录选**年均气温（环流）**：极区不再是一整块 −18 °C 的平台，南北极向平衡
值靠拢；45–55° 的冷带消失。选**海表温度**：极区海面为 −2 °C 而非 +10 °C，
极冠边界即 −2 °C 等值线。选**年降水量（环流）**：40–50° 的倾倒带减弱、极区
出现微量降水。选**洋流**：陆地上不再有残留流速。

季节循环已经出现且半球符号正确，但振幅只有地球的一个零头：压缩比上限被
32 seed 冷启动扫描钉在 30（设计 §6.6）。赤道辐合带仍缺失（设计 §6.5）。
