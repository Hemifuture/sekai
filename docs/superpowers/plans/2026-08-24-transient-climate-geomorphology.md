...
**Goal:** 用连续海岸、过程守恒和动态平衡当前态替代任意 100 ka 历史与失败的
终态不动点，再在可信径流上完成河网标定与 UI 交付。

**Architecture:** `world` 保存统一水面几何、当前水库/通量与终端平衡报告；
`generators` 以一个水面算子服务 P3/P4/P5，并从 P3 初始化一次未发布求解态。
P5 用 pseudo-transient continuation 求固定强迫下的动态平衡，所有中间态只活
在一次原子构建内。地图、球面和摘要只读同一当前态 payload。
...
- 新常量先用生产算子测量，再把语料、来源和取值过程写入规格修订；不得先钉
  数字再倒补依据。
- 经验地球范围只做 evidence；守恒、非负、拓扑、动态平衡、schema 和身份是
  hard gate。
...
- 修改：规格 R3 当前态修订与平衡实测修订

- [x] 完成旧固定 `dt/dt/2/dt/4` 九组 Release 探针；R3 将完整数据登记为固定
  `100 ka` 语义与产品性能的否决证据，不冻结时间常量。
- [ ] RED：snapshot/checkpoint/UI 不含累计年份、窗口历史或累计物质吞吐；测试
  必须拒绝旧时间型 wire，并证明取消/失败不发布中间态。
- [ ] 将累计 `FormationElevationComponents` 改为
  `primary_relief + equilibrium_adjustment = current_elevation`；另行发布当前
  过程速率/通量、当前沉积库存和年度固体通量账本，不双写历史字段。
- [ ] 把 `solve_geomorphic` 拆为一次初始化、一个可回滚的 current-state residual
  算子和一个 pseudo-transient continuation 更新；伪时间只属私有数值工作量。
- [ ] 先跑 seed 3/7/42 原始物理残差探针：固体通量平衡、净地表变化率、relief/
  平均高程/降水/径流/产沙漂移和不同 continuation 调度的终态差；据 R3 来源与
  实测冻结尺度、连续通过次数和工作上限，找不到直接来源的列为开放问题。
- [ ] 终端候选上重求统一水面、完整 P4、水量和水文；只有全部平衡/守恒/拓扑
  门禁同时通过才原子发布，工作上限只返回携带最佳残差的 typed failure。
- [ ] `FormationSolveReport` 只报告终端平衡残差、迭代/P4 solve 数、当前通量
  闭合、内存与确定性身份；不得出现模拟年份或时间窗。
- [ ] 解析 stream-power 稳态、不同 continuation 调度同终态、seed 7 无二周期、
  取消原子性和三档性能通过后提交。

提交：`Solve climate and landforms as a current state`
...
- [ ] 实现雪、单层土壤和慢地下水月平衡；重复当前气候年直至季节周期首尾
  库存漂移收敛，只发布当前周期状态。实际 ET 受能量需求与可用水共同限制，
  `Q_fast + Q_base` 成为唯一 P5 径流事实。
...
- [ ] 摘要逐位复制当前态 solve report 和水圈 budget，显示动态平衡残差、当前
  通量、库存分配及水量闭合；app 不重算公式或数值，不显示历史年份或窗口。
- [ ] 现有“海水量”更名“可迁移水库存”，另显示实际海洋体积；不增加时间步、
  容差或求解器旋钮。
- [ ] 检查窄/宽桌面与移动视口无重叠、无截字，地图/球面切换字段一致。
- [ ] focused 测试、三道门禁并提交。

提交：`Expose current-state hydrosphere diagnostics`
...
- [ ] 记录海岸连续性、解析稳态、不同 continuation 调度终态一致性、总水/当前
  固体通量闭合、河网检测、三档 P4/P5 wall clock、High memory 与取消延迟。
...
提交：`Record current-state hydrosphere delivery evidence`

## 每项承重技术的出处

- 独立时间积分参考：Hairer, Nørsett & Wanner (1993), *Solving Ordinary
  Differential Equations I*；Task 2 必须以实际 classic RK3 为 reference。
- 地形—气候动态平衡及可观测指标：Paik & Kim (2021), DOI
  `10.5194/hess-25-2459-2021`；抬升—侵蚀稳态：Whipple & Tucker (1999),
  DOI `10.1029/1999JB900120`。
- 隐式 stream-power：Braun & Willett (2013), DOI
  `10.1016/j.geomorph.2012.10.008`；稳态伪时间 continuation：Kelley & Keyes
  (1998), DOI `10.1137/S0036142996304796`。
...
- 全局陆面水量：Müller Schmied et al. (2021), WaterGAP v2.2d, DOI
  ...
  `10.1029/2021JF006239`；水库 spin-up 与漂移判稳：Cosgrove et al. (2003),
