...
Evidence (2026-08-09, Fix Round 1 final fresh Release run, product default seed 42, requested 20,000 / resolved exactly 20,252 cells): Windows x86_64, Intel64 Family 6 Model 183 Stepping 1, rustc 1.97.1, NVIDIA GeForce RTX 4080 SUPER via Vulkan. Equal Earth was 91.0183 ms / 17,301,504 B; unit globe 42.0275 ms / 12,672,808 B; locator 39.9583 ms / 10,854,112 B; field layers 253.1 us / 405,644 B; medium wind 3.3622 ms / 279,888 B; combined 176.6194 ms / 41,513,956 B. The test asserts the shared product default space specification, exact seed 42/source identity, elevation fill, preliminary prevailing wind overlay, and Medium glyph LOD. Initial GPU upload was 13,568,464 B and the static second-frame large-upload delta was exactly zero.
...
Evidence: the required-GPU 192x96 `Rgba8UnormSrgb` suite passed on the audited RTX 4080 SUPER Vulkan and RTX 4080 SUPER/PCIe/SSE2 OpenGL adapter/backend pairs. Exact hashes are enforced only for those audited pairs. Any other adapter still runs the complete CPU semantic oracle, render, and readback and prints every actual hash as `semantic-only-unaudited`; `SEKAI_REQUIRE_SPHERICAL_GPU=1` makes unavailable GPU infrastructure fail but does not mislabel an unknown adapter hash as audited. CPU oracles exhaustively lock every authoritative scalar/category/vector value and compact key, semantic cell/edge ID and cardinality, map/globe edge and vector GPU instance field, east/north direction, magnitude/normalization/color/length, phase-only animation immutability, unit radius, seam and pole semantics, front/back classification, and the complete sealed source-bound diagnostic mask. Negating a production vector direction produced RED before hash comparison; restoring it made the semantic oracle and both audited backends GREEN. BLAKE3 hashes are:
...
- map/globe diagnostics (sealed crate-private source-bound fixture): `dff1205540d94745dbb36fd43f64f882afdfec43385f1c4dd6d53fe10791662c` / `55136898b70af3a770a973686f0a7bc500cd57bc1f10c97af4916f43d07e6574`
...
Evidence: the exact fmt/check/clippy/workspace-all-targets-and-features/doc/Release-performance/wasm commands above passed fresh after Fix Round 1 on 2026-08-09; the workspace run exited 0 in 241.3 s. A supplemental fresh full run with `SEKAI_REQUIRE_SPHERICAL_GPU=1` exited 0 in 174.6 s (335 library tests plus every workspace target), proving the GPU target did not skip. The final Vulkan and OpenGL focused golden runs each passed 5/5; the OpenGL sealed diagnostic test passed separately. `git diff --check` passed; the pre-commit non-clean status contained only the intentional six-file Fix Round 1 patch, and `git ls-files .superpowers/sdd` remained empty.
...
Evidence: Step 4 has valid visible native and browser measurements, the original final exact automated gates and boundary audit passed in `beb25a7e47b30c35696d4931a4e2e24e2ad0d1af`, and Fix Round 1 preserves those runtime visuals while hardening packet revision safety, semantic/adapter golden policy, and the exact product-default performance fixture in a separate review-fix commit.
