...
- [x] Task 5 —— 产品级时延门与证据刷新
      产品级实测（走生产 stage graph，含质量评估与束校验）：**Draft 17.8 s，
      Standard 50.2 s**（seed 42；此前 19.2 / 53.4 s 是 Task 4 之前的值）。
...
1. Draft 档应在 20 s 内出图（产品级实测 17.8 s；机器波动约 ±2 s）。
2. Standard 档应在 60 s 内（产品级实测 50.2 s）。
