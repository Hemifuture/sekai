...
- [x] Task 6 —— 结构性 T0 去规则化决策门：Task 4R–5 上屏后与用户
      ...
      验证：用户决定被显式记录。
      **决定（2026-08-20，用户）：**
      ① 议题 A 推迟豁免——T0 去规则化不在 M1/M2 执行，
      所有同类噪声扰动项归入下方「T0 去规则化统一清单」，
      项目末期一次性执行并统一刷新演化/P5 证据与指纹。
      ② 议题 B：smoothstep 插值保留（它是 T1 `sample()` 把
      T0 离散格元值变成连续场的核心步骤，与已退役的烘培
      贴图无关，细分取色与河谷雕刻均依赖它）；平台副作用
      的处置定为 M2 深层级上屏后的校准期先行对策 1
      （细节振幅配比复核，免费），不足再升级对策 2
      （T0 场预平滑，需 A5 修订 + T1 指纹刷新）。

      **T0 去规则化统一清单（末期一次性执行，改动产物
      指纹的噪声扰动类工作全部归此）：**
      1. P2v5 板块边界域扭曲粗糙化（边界几何在演化期即
         不规则）；
      2. 小尺度洋壳年龄扰动（洋脊条带去平行化）；
      3. （备选，若 M2 校准期升级到对策 2）T0 场球面预
         平滑——仅改 T1 指纹，不动 T0 证据；
      后续发现的同类项追加到本清单而非即时执行。

      **T0 陆面高程谱（hypsometry）校准——已测量、另立任务
      （2026-08-20，用户报告"全球看不到山脉"后的探针结论）：**
      造山带在数据中真实存在且量级健康（草稿档两种子实测：
      演化构造高程在造山格元 p99 达海拔 +5.5~5.8 km，经 P5
      侵蚀后仍存 p99 +4.1 km、最高 +5.9~7.8 km；用户种子
      ≥2000 m 面积占球面 ~13%）。看不见的主因是**陆地整体
      呈高台**：陆地中位海拔 +1.2~1.6 km（地球约 +0.8 km），
      ≥500 m 占陆地 91%+，<200 m 的沿海低地不足 2%（地球有
      大片低地平原）；洋盆中位深度仅 −2.1~−2.3 km（地球
      −3.7 km）。水量库存按地球等比（scaled_earth_ocean_
      inventory_m3），故根因在固体高程谱：大陆干舷与洋盆
      深度分布对地球参考的偏离（静力学/地壳厚度密度、
      侵蚀-沉积低地生成）。校准改变 T0 产物与指纹，须另立
      规格修订并刷新演化/P5 证据，不在 M1/M2 执行；呈现侧
      （色带阶梯、光影、局部对比）的减症属 M2 Task 5。
- [x] Task 7 —— 门禁与验收：fmt/clippy/wasm/全量回归、计划勾选核对、
      撰写用户验收步骤、审计报告 artifact 补充前后对照截图。
      验证：门禁全绿；用户亲自走完验收步骤。
      （2026-08-20 完成：发布档全量回归 1,519 通过 / 0 失败
      （177 个测试目标），fmt / clippy -D warnings / wasm 全绿；
      前后对照与验收步骤发布为《Sekai M1 交付报告》
      artifact（旧审计报告保持原样，以新报告承载补充，避免
      整页替换风险）。用户验收步骤如下，最终验收归用户：
      1) 启动应用等待自动构建完成（标准档约 3–6 分钟；构建
         期间勿改显示设置，否则构建被保护性丢弃需重建）；
      2) 填色选「当前地表高程」：细分马赛克 + 蓝色河网；
      3) 大陆上滚轮放大：光标锚定、可至 4096×；
      4) 填色切「多年平均流量」「斯特拉勒河级」对照河线
         走向一致；
      5) 点击任意格元：实体检查正常；
      6) 切投影/拖中央经线：细分层与河线即时重投影；
      7) 三维球体重复 2–3（球体缩放上限 64×）；
      8) 填色切「板块编号」：回到权威格元填色；
      9) 档位切草稿重建：子单元变约 40 km。
      任何裂缝、断色、黑屏、点击失灵或报错均视为不通过。）
...
- （已修复 2026-08-20）材料守恒容差误校：控制面厚度以
  f32 存储并逐步量化，而账本以 f64 累计，量化差在
  ~1e5 格元×多轮演化下自然达 1e-5 量级（实测 22 板块/
  活跃/种子 2298831511111284992 为 9.9e-6）；旧界 1e-9 按
  全 f64 心智设定，把健康世界判死（“新种子并重建”
  报 build failed）。处置：控制/权威两个硬失败界重校
  为 1e-4（f32 量化底线之上一个量级余量，真会计缺口
  在百分比量级仍能捕获），证据仍如实记录实测残差；
  同时 worker 错误信息透传报告内的 Error 诊断（此前只显示
  “inspect the report diagnostics”却无处可看）。
- （已修复 2026-08-20）per-world 质量指标不再是运行时硬
  门禁：P2/P3/P4/P5 四处 quality 报告校验曾要求每个指标
  必须 Pass，否则拒绝发布世界（用户撞到：P4
  orographic-uplift-enrichment-ratio 在某些世界上必然 Fail
  ——山少或风向不巧是世界特征而非数据错误）。用户裁定
  （2026-08-20）：统计特征指标是对世界的测量，任何
  状态都是合法证据；结构性校验（绑定、指标集、锁定
  界、样本数）保持硬失败。开发期证据测试（固定种子）
  仍独立断言 Pass，门禁活在测试里而非运行时。
