...
- [ ] Task 4R —— 格元内测地细分放大视图（替代烘焙贴图；用户批准的
      ...
      **视觉语言修正（2026-08-20，用户反馈）**：自始至终只有一种
      ...
      留待用户明确需要时另行开启。
- [x] Task 5 —— 河流雕刻与河道：把 P5 已发布河段以解析河谷剖面写入
      `sample()`，并在放大视图中绘制河段折线（线宽按斯特拉勒河级）。
      验证（UI）：河流走向与检查器报告的汇流一致，河谷在放大地形上
      可见下切。
      （实现 2026-08-20：雕刻按规格 §7 与修订 A4 写入
      `sample()`（水力几何宽深、拓扑 running-min 单调河床、
      大圆弧精确最近点、min 只降不筑坝），四项不变量模块
      测试守门；河道折线以符号化像素宽（1.2 + 0.7×(河级
      −1)，钳 6 px）在地图与球体随地表高程视图自动显示，
      跨接缝段从地图中丢弃（段长仅一个格距，缝缘缺口为
      细条）。诚实预期：M1 的 40–44 km 显示图元下河谷几何
      下切（百米至数千米宽）基本不可见，雕刻的视觉验收
      依赖 M2 深层级；本阶段 UI 验收以河道折线走向与汇流
      一致性为准。最终验收归用户。）
      随附相机修复（2026-08-20，用户报告）：① 2D 地图滚轮
      缩放改为以鼠标位置为锚点（pan' = pan·factor +
      anchor·(1−factor)/2，锚点下的世界点不动）；② 相机
      最大倍数 MAX_ZOOM 64 → 4096（最细显示图元可满屏，为
      M2 留余量），pan 上界随 zoom 缩放，球体 MAX_SCALE 8 →
      64；持久化相机校验改为先 zoom 后 pan。
...
3. 寻路定位：连续移动语义以三角形层级为 navmesh（出处：
   ...
   或 M2 分块）落地时再做。
4. 河道几何的分层细化（2026-08-20 与用户讨论得出）：
   蠕蜒是物理事实且分形（波长 ≈ 10–14 × 河宽，
   Leopold & Wolman 1957/1960；分维 ~1.1–1.3）。原则：
   几何细节走层级细化，物理事实留在 L0——河的存在性、
   拓扑与水量永由 P5 权威；子层只做确定性重路由（每层连
   该层子图元质心/边，沿 T1 高程向低处走 + 段 ID 种子扰动，
   端点对接、限定母段走廊），雕刻读当层路径保持同源。
   出处：Fournier, Fussell & Carpenter 1982（随机中点细分）；
   Génevaux et al. 2013（层级排水图）。先行步（可选，不会
   白做）：地形引导的递归中点蠕蜒 = 重路由的第 0 版；
   完整版属 M2（与按需细分同期）。同一原则适用于海岸线
   （已由 T1 warp 实现）与未来道路/边界。
