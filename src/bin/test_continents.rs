...
use sekai::terrain::{TerrainConfig, TerrainGenerator, TerrainGenerationMode};
...
fn height_to_color(h: f32) -> (u8, u8, u8) {
    ...
    if h <= sea_level {
        let t = h / sea_level;
        ((10.0 + 40.0 * t) as u8, (30.0 + 70.0 * t) as u8, (100.0 + 100.0 * t) as u8)
    } else if h < 60.0 {
        let t = (h - sea_level) / 40.0;
        ((80.0 + 40.0 * t) as u8, (180.0 - 20.0 * t) as u8, (80.0 - 30.0 * t) as u8)
    } else if h < 120.0 {
        let t = (h - 60.0) / 60.0;
        ((120.0 + 60.0 * t) as u8, (160.0 - 40.0 * t) as u8, (50.0 - 20.0 * t) as u8)
    } else if h < 180.0 {
        let t = (h - 120.0) / 60.0;
        ((180.0 - 40.0 * t) as u8, (120.0 - 20.0 * t) as u8, (30.0 + 70.0 * t) as u8)
    } else {
