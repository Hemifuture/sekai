...
impl FeatureDetector {
    ...
    pub fn detect_features(
        ...
        while let Some(start) = feature_ids[search_start..].iter().position(|&id| id == 0).map(|p| p + search_start) {
    ...
    pub fn get_coastline_cells(
        ...
        for (i, &h) in heights.iter().enumerate() {
            ...
            // 检查是否有海洋邻居
            let has_ocean_neighbor = neighbors[i].iter().any(|&n| {
                heights[n as usize] < SEA_LEVEL
            });
    ...
    pub fn smooth_coastline(
        ...
        for _ in 0..iterations {
            ...
            for (i, &h) in original.iter().enumerate() {
                ...
                // 统计同类型邻居数量
                let same_type_count = neighbors[i].iter().filter(|&&n| {
                    (original[n as usize] >= SEA_LEVEL) == is_land
                }).count();
    ...
    ///
    /// 返回每个单元格允许的最大噪声幅度
    pub fn calculate_noise_constraints(
        &self,
        heights: &[u8],
        distance_field: &[i8],
    ) -> Vec<f32> {
...
///
/// 确保噪声不会改变海陆类型
pub fn apply_constrained_noise(
    heights: &mut [f32],
    noise: &[f32],
    constraints: &[f32],
) {
...
mod tests {
    ...
    fn test_coastline_detection() {
        ...
        let neighbors: Vec<Vec<u32>> = (0..16).map(|i| {
            let row = i / 4;
            let col = i % 4;
            let mut n = Vec::new();
            if row > 0 { n.push((i - 4) as u32); }
            if row < 3 { n.push((i + 4) as u32); }
            if col > 0 { n.push((i - 1) as u32); }
            if col < 3 { n.push((i + 1) as u32); }
            n
        }).collect();
