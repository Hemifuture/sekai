...
use sekai::world::natural::{
    ClimateSpec, GeologicSpec, TectonicActivity, TectonicSpec, TECTONIC_SPEC_SCHEMA_V1,
};
...
fn complete_natural_graph_publishes_physical_artifacts_with_exact_stage_metadata() {
    ...
    let relief_descriptor = &graph.descriptors()[9];
    assert_eq!(
        relief_descriptor
            .dependencies()
            .iter()
    ...
    let geologic_descriptor = &graph.descriptors()[10];
    assert_eq!(
        geologic_descriptor
            .dependencies()
            .iter()
    ...
    let climate_descriptor = &graph.descriptors()[11];
    assert_eq!(
        climate_descriptor
            .dependencies()
            .iter()
...
fn complete_natural_graph_cache_tracks_transitive_tectonic_changes() {
    ...
    let changed = engine
        ...
        .unwrap();
    assert_eq!(changed.report.cache_hits(), 6);
...
fn cache_projected_spec_change_invalidates_only_natural_downstream() {
    ...
    assert_eq!(changed.report.cache_misses(), 10);
    assert_eq!(
        changed
            .artifacts
...
fn cache_rule_failure_publishes_nothing_and_preserves_prior_valid_entries() {
    ...
    assert_eq!(
        cache.len(),
        16,
        "the independent climate and geologic audits and projections remain valid"
    );

    let recovered = engine
        .build(RootSeed::new(42), complete_external(12), &mut cache)
        .unwrap();
    assert_eq!(recovered.report.cache_hits(), 12);
